  ...
  if (shape !== null) {
    ...
    for(let i=0;i<this.shapes.length;i++)
     ...
     async saving()
      {
        ...
        if(this.savjason){
        await fetch('http://localhost:8081/saveJson', {
          method: 'POST',
          body: this.path+".json",
        }).catch(error => {
          console.error('Fetch error:', error);
      ...
      if(this.savxml){
        await fetch('http://localhost:8081/saveXml', {
          method: 'POST',
          body: this.path + ".xml",
        }).catch(error => {
          console.error('Fetch error:', error);
     ...
     async loading(){
      ...
      this.redos = 0 
        if(this.lodjason){
        await fetch('http://localhost:8081/loadJson', {
          method: 'POST',
          body: (this.path + ".json"),
        })
        .then(res => res.json())
      ...
      if(this.lodxml){
        this.undos = 0
        this.redos = 0
        await fetch('http://localhost:8081/loadXml', {
          method: 'POST',
          body: this.path+".xml",
        })
        .then(res => res.json())
